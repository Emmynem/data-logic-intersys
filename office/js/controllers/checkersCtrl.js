chuks_project.controller('checkersCtrl',function($scope,$rootScope,$http,$timeout,$location,$route,$window,storage,notify){
  ...
  $scope.edit_checker_details = function (user_uuid, fullname, gender, phone_number, access) {
    $scope.checker.fullname = fullname;
    $scope.checker.gender = gender;
        ...
        $http.post('server/edit_checker_details.php', $scope.editCheckerDetailsData)
          .then(function (response) {
            if (response.data.engineMessage == 1) {
              ...
              notify.do_notify($scope.result_u, "Edit Activity", "Data Checker details edited");
  ...
  $scope.edit_checker_password = function (user_uuid) {
        ...
        $scope.continue_action = function () {
          ...
          .then(function (response) {
            if (response.data.engineMessage == 1) {
              ...
              notify.do_notify($scope.result_u, "Edit Activity", "Checker password edited");

<?php
  ...
  if ($connected) {
    try {
      ...
      $sql = "SELECT access FROM users_data WHERE user_uuid=:user_uuid";
      $query = $conn->prepare($sql);
      $query->bindParam(":user_uuid", $data['user_uuid']);
      $query->execute();

      $sql2 = "SELECT access FROM users_data WHERE user_uuid=:user_uuid";
      ...
      if ($query->rowCount() > 0 && $access == 1) {
        ...
        $sql2 = "UPDATE users_data SET access=:access, last_modified=:last_modified WHERE user_uuid=:user_uuid";
        $query2 = $conn->prepare($sql2);
        $query2->bindParam(':user_uuid', $data['user_uuid']);
